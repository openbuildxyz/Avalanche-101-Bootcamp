...
## 进阶项：IOC / FOK 订单支持（8 分）

本项工作对应 8 分进阶评分项。实现提交：[c538c46 — [Task7] Add IOC and FOK order support](https://github.com/kKassidy/Mini-DEX/commit/c538c46)。

### 实现范围与行为

- 新增可选 `timeInForce: GTC | IOC | FOK`。省略时保持兼容：限价单默认 GTC，市价单保留原有类似 IOC 的行为。支持显式市价 IOC；市价 GTC / FOK 在锁定余额前拒绝，本实现的 FOK 仅支持限价单。
- IOC 沿用价格 / FIFO 撮合，允许部分成交，未成交余量不进入订单簿；现有预留记账精确释放未使用的锁定金额，BUY 退款包含适用的 maker 价格改善差额。
- FOK 立即全量成交或完全不成交：沿实际价格 / FIFO 执行路径进行只读预检，合格流动性不足时，在分配序号或修改订单簿前返回零成交且不挂单，并释放该 incoming order 的全部预留金额。此流动性预检不保证任意结算运行时错误具有事务性。
- 保持原有自成交原子拒绝策略：IOC / FOK 使用相同的只读执行路径遍历，预期执行路径遇到自身流动性时，在任何修改前拒绝整个 incoming order；实际执行路径之外的自身流动性不触发拒绝。

### 验证证据

- Server 完整测试：6 个文件、114 个测试通过；Web 测试：1 个文件、10 个测试通过。
- Server / Web typecheck 及 `git diff --check` 均通过。
- 覆盖 BUY / SELL、IOC 全成 / 部成 / 无成交、FOK 全成及流动性不足时零修改、价格限制、同价 FIFO 与多档成交、自成交原子拒绝、精确锁定 / 退款、BUY 价格改善退款、其他既有锁定金额、省略 TIF 的向后兼容、市价 IOC 兼容，以及无效 / 不兼容 TIF 在锁定前拒绝。
