...
describe("OrderBook", () => {
  ...
  it("时间优先：买侧同价按提交顺序成交", () => {
    const ob = new OrderBook();
    const first = ob.submit(limit("a", "buy", "100", "1")).resting!;
    ob.submit(limit("b", "buy", "100", "1"));
    const r = ob.submit(market("t", "sell", "1"));
    expect(r.fills).toHaveLength(1);
    expect(r.fills[0]!.makerOrderId).toBe(first.id);
    expect(r.fills[0]!.maker).toBe("a");
  ...
  it("拒绝 self-trade：同地址的挂单不会被自己成交", () => {
    const ob = new OrderBook();
    ob.submit(limit("alice", "sell", "100", "1"));
    const own = ob.submit(limit("alice", "buy", "100", "1"));
    expect(own.fills).toHaveLength(0);
    expect(own.resting?.remaining).toBe(F("1"));
    const other = ob.submit(limit("bob", "buy", "100", "1"));
    expect(other.fills).toHaveLength(1);
    expect(other.fills[0]!.maker).toBe("alice");
    expect(other.fills[0]!.taker).toBe("bob");
  });

  it("拒绝 self-trade：市价单也不会吃自己的挂单", () => {
    const ob = new OrderBook();
    ob.submit(limit("alice", "sell", "100", "1"));
  ...
  it("拒绝 self-trade：整档都是自己的单时跳到下一档", () => {
    const ob = new OrderBook();
    ob.submit(limit("alice", "sell", "100", "1"));
    ...
    expect(r.fills[0]!.price).toBe(F("101"));
    expect(ob.bestAsk()).toBe(F("100"));
  });

  it("拒绝 self-trade：同档跳过自己的单，其他人仍按时间优先", () => {
    const ob = new OrderBook();
    ob.submit(limit("alice", "sell", "100", "1"));
    const bob = ob.submit(limit("bob", "sell", "100", "1")).resting!;
    ob.submit(limit("carol", "sell", "100", "1"));
    const r = ob.submit(limit("alice", "buy", "100", "2"));
    expect(r.fills.map((f) => f.maker)).toEqual(["bob", "carol"]);
    expect(r.fills[0]!.makerOrderId).toBe(bob.id);
    expect(ob.ordersOf("alice").some((o) => o.side === "sell")).toBe(true);
  });
