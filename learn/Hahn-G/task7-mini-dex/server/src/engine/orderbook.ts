...
export class OrderBook {
  ...
  submit(input: Omit<Order, "remaining" | "seq" | "ts"> & Partial<Pick<Order, "ts">>): { fills: Fill[]; resting: Order | null } {
    ...
    // 在撮合造成任何状态变化前拒绝自成交，保证失败是原子的，原有挂单不受影响。
    if (this.wouldSelfTrade(order)) throw new Error("OrderBook: self-trade rejected");
  ...
  /** 如果本次订单会与同一 owner 的任一对手单交叉，则整笔拒绝。 */
  private wouldSelfTrade(taker: Order): boolean {
    ...
    for (const price of opposite.prices) {
      ...
      if (opposite.book.get(price)!.orders.some((maker) => maker.owner === taker.owner)) return true;
    }
    return false;
  }
