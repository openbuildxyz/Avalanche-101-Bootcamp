...
describe("OrderBook", () => {
  ...
  it("拒绝 self-trade：新订单会与自己的挂单成交时整单拒绝", () => {
    const ob = new OrderBook();
    const maker = ob.submit(limit("alice", "sell", "100", "1")).resting!;

    expect(() => ob.submit(limit("alice", "buy", "100", "1"))).toThrow(/self-trade/);
    expect(ob.get(maker.id)?.remaining).toBe(F("1"));
    expect(ob.snapshot()).toEqual({ bids: [], asks: [[F("100"), F("1")]] });
  });

  it("self-trade 预检只看实际会成交的数量", () => {
    const ob = new OrderBook();
    ob.submit(limit("bob", "sell", "99", "1"));
    ob.submit(limit("alice", "sell", "100", "1"));
